...
const EmployeeCustomizationReport = () => {
  ...
  const getYearlySales = async () => {
    const res = await fetch(
      `http://localhost:3000/api/admin/reports/customization/yearly?` +
        new URLSearchParams({
          reportType: reportType.value,
          ...
          <Bar
            options={{
              ...
              plugins: {
                title: {
                  display: true,
                  text: `Top ${reportType.name} on ${month.month} ${year}`,
                },
                legend: {
                  display: false,
                },
            ...
            data={{
              labels: reportDatas.map((i) => i.name), // ['prod1', 'prod2']
              datasets: [
                {
                  label: "Sales",
                  data: reportDatas.map((i) => i.totalQuantity), // ['80', '70']
                  backgroundColor: [
                    "#5FBDFF",
                    "rgba(2,138,136, 0.6)",
                    "rgba(255, 205, 86, 0.6)",
          ...
          <Bar
            options={{
              ...
              plugins: {
                title: {
                  display: true,
                  text: `Top ${reportType.name} ${dayjs(startDate).format(
                    "MMM YYYY"
...
export default EmployeeCustomizationReport;
