      ...
      <div className="flex items-center py-4">
        ...
        {!columnSelected && (
          <Input
            ...
            value={table.getColumn("flavorId")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table.getColumn("flavorId")?.setFilterValue(event.target.value)
            }
            className="max-w-sm w-fit ml-4"
          />
        )}
        {/* if EMPLOYEE ID is selected */}
        {columnSelected == "flavorId" ? (
          <Input
            placeholder="Filter flavor id..."
            value={table.getColumn("flavorId")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table.getColumn("flavorId")?.setFilterValue(event.target.value)
            }
        ...
        {/* if EMPLOYEE ID is selected */}
        {columnSelected == "flavorName" ? (
          <Input
            placeholder="Filter flavor name..."
            value={table.getColumn("flavorName")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table.getColumn("flavorName")?.setFilterValue(event.target.value)
            }
            className="max-w-sm w-fit ml-4"
          />
        ) : null}{" "}
        {/* if userRole is selected */}
        {columnSelected == "flavorPrice" ? (
          <Input
            placeholder="Filter flavor price..."
            value={table.getColumn("flavorPrice")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table.getColumn("flavorPrice")?.setFilterValue(event.target.value)
            }
            className="max-w-sm w-fit ml-4"
          />
        ) : null}
        {/* if userRole is selected */}
        {columnSelected == "flavorStatus" ? (
          <Input
            placeholder="Filter flavor price..."
            value={table.getColumn("flavorStatus")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table
                .getColumn("flavorStatus")
...
export default CustomizationFlavorsTable;
