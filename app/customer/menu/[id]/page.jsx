...
export default async function Menu(path) {
  const { params } = path;
  const { id } = params;
  ...
  const sprinkles = await getSprinkle();
  const shapes = await getShape();
  const flowers = await getFlower();

  const handleProducts = () => {
    const productsWithSizes = productsData.map((prod) => {
      const { productId, isRemoved } = prod;
